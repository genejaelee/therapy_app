	...
	presenceChannel.bind('send_message', function(message) {
		...
		if(user_id == message.user.id) {
			...
			if(!hasFocus) {
				...
				document.title = "New Message! - Scouter"
			}
		}
		...
		$('#messages').append('<li class="' + who + 'just_added_id_' + message.id + '" style="display:none;"><span><strong>' + message.user.nickname + '</strong> said:</span>' +
													'<span class="timestamps">' + message.f_created_at + '</span>' +
													'<br>' + 
													'<div class="message-wrapper"><div class="message">' + 
														'<p>' + replaceURLWithHTMLLinks(message.message) + '</p></div></div></li>');
